#!/usr/bin/env python3
"""Verify docs/part5/20-medical-imaging-triage.md.

Every section in this chapter (20.1 through 20.5) is pure, deterministic
self-test against synthetic fixtures and synthetic data only -- no real
patient data or real medical image appears anywhere in this chapter, and,
like Chapter 19, no section here has an external system library missing
from the cross-compilation sandbox, so this script recompiles and reruns
all five sections without any documented reduced-verification exception.

Sections 1, 2, 3, and 4 compile with plain flags. Section 5 alone reuses
Chapter 18.2's own vision-encoder machinery, so it needs -ffp-contract=off
for the same cross-architecture floating-point determinism reason Chapter
19.2 needed it -- but, unlike Chapter 19.2, Section 5 does not touch
std::mdspan at all, so it needs no vendored mdspan include path.
"""
import re
import subprocess
import sys
...
MD_PATH = BASE / "docs/part5/20-medical-imaging-triage.md"

EXPECTED_H1 = "# Chapter 20: Medical Imaging Triage: Preliminary Radiology Reads and Human-in-the-Loop Workflow"
EXPECTED_H2 = [
...
COMPILE_FLAGS = {
    ...
    5: ["-std=c++23", "-Wall", "-Wextra", "-O2", "-ffp-contract=off"],
}
...
def main() -> int:
    ...
    if len(bash_blocks) != 5:
        ...
        for idx in range(1, 6):
            ...
            needs_fp_contract_off = idx == 5
            has_fp_contract_off = "-ffp-contract=off" in block
            if needs_fp_contract_off and not has_fp_contract_off:
                fail(f"bash block {idx} is section {idx}, which requires -ffp-contract=off")
            if not needs_fp_contract_off and has_fp_contract_off:
                fail(f"bash block {idx} unexpectedly compiles with -ffp-contract=off "
                     f"(section {idx} has no vision-encoder code requiring it)")
            if "MDSPAN_IMPL_STANDARD_NAMESPACE" in block or "_vendor_mdspan" in block:
                fail(f"bash block {idx} unexpectedly references mdspan (no section in this "
                     f"chapter uses std::mdspan)")
    ...
    for idx, fname in FILES.items():
        src = BASE / fname
        binp = BASE / f"_verify_ch20_{idx:02d}_bin"
