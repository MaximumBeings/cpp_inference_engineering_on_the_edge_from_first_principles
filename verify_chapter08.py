#!/usr/bin/env python3
"""Verify docs/part2/08-memory-wall-and-roofline-model.md."""
import re
import subprocess
import sys
...
MD_PATH = BASE / "docs/part2/08-memory-wall-and-roofline-model.md"
MDSPAN_INCLUDE = BASE / "_vendor_mdspan/include"

EXPECTED_H1 = "# Chapter 8: The Memory Wall and the Roofline Model -- Why Arithmetic Is Rarely the Bottleneck"
EXPECTED_H2 = [
...
# (filename, needs_mdspan_flags)
FILES = {
    1: ("01_roofline_and_ridge_point.cpp", False),
    2: ("02_kernel_arithmetic_intensity.cpp", False),
    3: ("03_prefill_vs_decode.cpp", True),
    4: ("04_continuous_batching.cpp", False),
    5: ("05_full_layer_roofline_profile.cpp", True),
}
OUT_FILES = {
    1: "01_roofline_and_ridge_point.out.txt",
    2: "02_kernel_arithmetic_intensity.out.txt",
    3: "03_prefill_vs_decode.out.txt",
    4: "04_continuous_batching.out.txt",
    5: "05_full_layer_roofline_profile.out.txt",
}
...
def main() -> int:
    ...
    for idx, (fname, needs_mdspan) in FILES.items():
        src = BASE / fname
        binp = BASE / f"_verify_ch8_{idx:02d}_bin"
